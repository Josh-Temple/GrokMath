...
- Unit 16 is synchronized as `Draft` after the third bounded pass (frontmatter + `## Concept Map` + `## Intuitive Explanation` + `## Formal Definition` + `## Worked Examples` + `## Common Mistakes and How to Avoid Them` + `## Quick Checks` populated).
