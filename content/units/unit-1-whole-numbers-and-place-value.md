...
1. Compare digits from the greatest place value to the least.
2. At the first place where digits differ, the number with the greater digit is greater.
3. If all corresponding digits are equal, the numbers are equal.

To round to a target place:
1. Identify the target place digit.
2. Look at the digit one place to the right.
3. If that digit is 5–9, increase the target digit by 1.
4. If that digit is 0–4, keep the target digit the same.
5. Replace all digits to the right of the target place with 0.
